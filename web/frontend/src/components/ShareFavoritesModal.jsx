...
import { shareFavorites, waitForJob, describeError, isAbortError } from "../api/client";

export default function ShareFavoritesModal({ deals, onClose }) {
  ...
  const handleSend = async () => {
    ...
    setProgress("");
    try {
      // Only sale_ids are sent; the server emails the analyses it has stored.
      const { job_id } = await shareFavorites({
        recipient_name:  recipientName.trim(),
        recipient_email: recipientEmail.trim(),
        sender_name:     senderName.trim() || undefined,
        note:            note.trim() || undefined,
        sale_ids:        deals.map(d => d.sale_id),
      });
      ...
      <div className="bg-white rounded-xl border border-brand-line shadow-xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-brand-line">
          <h2 className="text-base font-bold text-brand-charcoal">
            Send Saved Properties Analysis
          </h2>
          <button
            onClick={onClose}
        ...
        {status === "success" ? (
          <div className="px-5 py-8 text-center space-y-3">
            ...
            <p className="text-sm text-gray-500">
              A report for{" "}
              <span className="font-medium">{deals.length} saved properties</span> was emailed to{" "}
          ...
          <div className="px-5 py-5 space-y-4">
            ...
            <div className="bg-brand-gray rounded-lg px-4 py-3">
              <p className="text-xs text-gray-500 mb-2">
                {deals.length} saved {deals.length === 1 ? "property" : "properties"}
              </p>
              <div className="space-y-1.5 max-h-32 overflow-y-auto pr-1">
                {deals.map((d, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="text-brand-orange font-bold text-xs mt-0.5 shrink-0">
                      {i + 1}.
                    </span>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-brand-charcoal leading-snug truncate">
                        {d.address}
                      </p>
                      <p className="text-xs text-gray-400">
                        {[d.municipality, d.verdict && `Verdict: ${d.verdict}`, d.score != null && `Score: ${d.score}/100`]
                          .filter(Boolean).join(" · ")}
                      </p>
                    </div>
                  </div>
            ...
            <div className="flex gap-3 pt-1">
              ...
              >
                {status === "sending" ? "Sending…" : `Send ${deals.length} Properties`}
              </button>
            </div>
          </div>
        )}
